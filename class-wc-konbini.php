...
/**
 * Oceanpayment Konbini Payment Gateway
 *
 * Provides a Oceanpayment Konbini Payment Gateway, mainly for testing purposes.
 *
 * @class 		WC_Gateway_Konbini
 * @extends		WC_Payment_Gateway
 * @version		1.0
 * @package		WooCommerce/Classes/Payment
 * @author 		Oceanpayment
 */
class WC_Gateway_Konbini extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'konbini';
        $this->icon               = apply_filters('woocommerce_konbini_icon', plugins_url( 'images/op_konbini.png', __FILE__ ));
        $this->has_fields         = false;
        $this->method_title       = __( 'Oceanpayment Konbini', 'woocommerce' );
        ...
        $this->description  = $this->get_option( 'description' ).$testnote;
        $this->instructions = $this->get_option( 'instructions', $this->description );

        // Actions
        add_action( 'woocommerce_api_wc_gateway_konbini', array( $this, 'check_ipn_response' ) );
        add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
        add_action( 'valid-konbini-standard-itn-request', array( $this, 'successful_request' ) );
        add_action( 'woocommerce_receipt_' . $this->id, array( $this, 'receipt_page' ) );
        add_action( 'woocommerce_thankyou_konbini', array( $this, 'thankyou_page' ) );
        ...
        add_action( 'woocommerce_receipt_konbini', array( $this, 'receipt_page' ) );
    }
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		if ( $this->instructions && ! $sent_to_admin && 'konbini' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => __( 'Enable Oceanpayment Konbini Card Payment', 'woocommerce' ),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default'     => __( 'コンビニ／銀行振込', 'woocommerce' ),
                'desc_tip'    => true,
            ),
            'description' => array(
    ...
    function receipt_page( $order ) {
        echo $this->generate_konbini_form( $order );
    }
     ...
     * 生成 Konbini form.
     */
    public function generate_konbini_form( $order_id ) {
        ...
        if(!empty($order->get_billing_first_name())){
            $billing_firstName = $this->OceanHtmlSpecialChars($order->get_billing_first_name());
        }elseif(!empty($order->get_billing_last_name())){
            $billing_firstName  = $this->OceanHtmlSpecialChars($order->get_billing_last_name());
        }else{
            $billing_firstName  = 'N/A';
        }
        //账单人姓
        if(!empty($order->get_billing_last_name())){
            $billing_lastName  = $this->OceanHtmlSpecialChars($order->get_billing_last_name());
        }elseif(!empty($order->get_billing_first_name())){
            $billing_lastName = $this->OceanHtmlSpecialChars($order->get_billing_first_name());
        }else{
        ...
        if(strlen($billing_phone) > 11){
            $billing_phone = substr($billing_phone,0,11);
        }elseif(0 < strlen($billing_phone) && strlen($billing_phone) < 10 ){
            $str = 10 - strlen($billing_phone);
            $billing_phone = $billing_phone.$this->splitPhone($str);
        }
        //账单人国家
        $billing_country   = !empty($order->get_billing_country()) ? $order->get_billing_country() : 'N/A';
        //账单人州(可不提交)
        $billing_state     = $this->get_konbini_state( $order->get_billing_country(), $order->get_billing_state() );
        //账单人城市
        ...
        //收货人的名
        $ship_firstName    = $this->OceanHtmlSpecialChars($order->get_billing_first_name());
        //收货人的姓
        $ship_lastName     = $this->OceanHtmlSpecialChars($order->get_billing_last_name());
        //收货人的电话
        $ship_phone 	   = str_replace( array( '(', '-', ' ', ')', '.' ), '', $order->get_billing_phone() );
        if(strlen($ship_phone) > 11){
            $ship_phone = substr($ship_phone,0,11);
        }elseif(0 < strlen($ship_phone) && strlen($ship_phone) < 10 ){
            $str = 10 - strlen($ship_phone);
            $ship_phone = $ship_phone.$this->splitPhone($str);
        }
        //收货人的国家
        $ship_country 	   = $order->get_shipping_country();
        //收货人的州（省、郡）
        $ship_state 	   = $this->get_konbini_state( $order->get_shipping_country(), $order->get_shipping_state() );
        //收货人的城市
        $ship_city 		   = $order->get_shipping_city();
        //收货人的详细地址
        $ship_addr 		   = $order->get_shipping_address_1();
        //收货人的邮编
        $ship_zip 		   = $order->get_shipping_postcode();
        //收货人email
        $ship_email        = $order->get_billing_email();
        ...
        $data_to_send .= "</div>";
        $data_to_send .= "</div>";
        $data_to_send .= "<form  method='post' name='konbini_checkout' action='".$this->settings['submiturl']."'  >";
        ...
        $data_to_send .= 'document.konbini_checkout.submit();' . "\n";
        $data_to_send .= '</script>' . "\n";
    ...
    function notice_payment( $order ) {
        ...
        if($this->xml_parser($xml_str)){
            ...
            $local_signValue  = hash("sha256",$return_info['account'].$return_info['terminal'].$return_info['order_number'].$return_info['order_currency'].$return_info['order_amount'].$return_info['order_notes'].$return_info['card_number'].
                $return_info['payment_id'].$return_info['payment_authType'].$return_info['payment_status'].$return_info['payment_details'].$return_info['payment_risk'].$this->settings['securecode']);
            ...
            if($return_info['response_type'] == 1){
                ...
                if(strtoupper($local_signValue) == strtoupper($return_info['signValue'])){
                    ...
                    if ($return_info['payment_status'] == 1) {
                        //成功
                        $order->update_status( 'processing', __( $testorder.$return_info['payment_details'], 'oceanpayment-konbini-gateway' ) );
                       wc_reduce_stock_levels( $order_id );
                        ...
                        if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    		$order->update_status( 'on-hold', __( $testorder.$return_info['payment_details'], 'oceanpayment-konbini-gateway' ) );
                		}
                        ...
                        if($errorCode == '20061'){
                            ...
                            if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    			$order->update_status( 'failed', __( $testorder.$return_info['payment_details'], 'oceanpayment-konbini-gateway' ) );
                			}
                        }
    ...
    function return_payment( $order ) {
        ...
        //支付链接
        $pay_barCode       = sanitize_text_field($_REQUEST['pay_barCode']);
        ...
        if(strtoupper($local_signValue) == strtoupper($back_signValue)){
            ...
            if ($payment_status == 1) {
                //成功
                $order->update_status( 'processing', __( $testorder.$payment_details, 'oceanpayment-konbini-gateway' ) );
                WC()->cart->empty_cart();
                ...
                if(empty($this->completed_orders()) || !in_array($order_number, $this->completed_orders())){
                	$order->update_status( 'on-hold', __( $testorder.$payment_details, 'oceanpayment-konbini-gateway' ) );
        		}
        		$url = '';
                header('Location:'.$pay_barCode);
                ...
                if($errorCode == '20061'){
                    ...
                    if(empty($this->completed_orders()) || !in_array($order_number, $this->completed_orders())){
                		$order->update_status( 'failed', __( $testorder.$payment_details, 'oceanpayment-konbini-gateway' ) );
        			}
    ...
    public function get_product($order,$type){
        ...
        foreach ($order->get_items() as $item_key => $item ){
            ...
            if($type == 'num'){
                ...
                $product->get_name() != '' ? $product_array[] = substr($product->get_name(), 0,22) : $product_array[] = 'N/A';
            }         
        }
     ...
     * 获取州/省
     */
    public function get_konbini_state( $cc, $state ) {
